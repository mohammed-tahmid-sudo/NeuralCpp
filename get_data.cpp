...
load_mnist_csv_debug(const std::string &path, bool normalize = true,
                     std::size_t expected_pixels = 784) {
  ...
  while (std::getline(file, line)) {
    ...
    while (std::getline(ss, cell, ',')) {
      ...
      cell.erase(
          std::find_if(cell.rbegin(), cell.rend(),
                       [](unsigned char ch) { return !std::isspace(ch); })
              .base(),
          cell.end());
      try {
  ...
  //   for (size_t j = 0; j < images[i].size() && j < 10; ++j)
  //     std::cout << images[i][j] << ' ';
  //   std::cout << '\n';
  // }
  //
...
// int main() {
//     auto [images, labels] =
//     load_mnist_csv_debug("/home/tahmid/NeuralCpp/data/mnist_test.csv", true);
